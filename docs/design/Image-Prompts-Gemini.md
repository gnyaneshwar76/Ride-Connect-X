...
3. Drop it in `frontend/app/src/main/res/drawable-nodpi/`.
...
```bash
python tools/images/prepare_vehicles.py --src "<originals>" --out frontend/app/src/main/res/drawable-nodpi
python tools/images/prepare_scenes.py   --src "<originals>" --out frontend/app/src/main/res/drawable-nodpi --store-out docs/store
```
...
`img_promo_keyart` lands in `brand/store/`, not in `res/` — it is store artwork
