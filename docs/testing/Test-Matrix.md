...
| A10 | Heartbeat cadence | UNTESTED | `tools/cluster/evidence.sh heartbeat` — one pulse per beat, not bursts |
...
| B5 | Display clears when navigation ends | UNTESTED | `tools/cluster/evidence.sh navend`. **Fix installed but never run** |
...
| # | Feature | Status | How to settle it |
|---|---|---|---|
| C1 | Missed-call lamp | UNTESTED | `tools/cluster/evidence.sh call`. **Filter rewritten 19 Sep, never seen a real call** |
| C2 | WhatsApp call lights the lamp | UNTESTED | Same capture, WhatsApp call |
| C3 | Message lamp — WhatsApp | UNTESTED | `tools/cluster/evidence.sh message` |
