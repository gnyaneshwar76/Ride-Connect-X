...
```
tools/cluster/sweep-hold.sh 37
```
...
**Step 1 — the Dammaiguda loop again** (`tools/gpx/rcx-hyd-loop.gpx`, saved in Lockito as
...
**Step 2 — every other turn, held one at a time** (`tools/cluster/sweep-hold.sh <code>`):
...
`tools/cluster/cluster-codes.csv`. Then one Lockito route through a few turns and a
