...
cd E:\RideConnetX
git checkout security-hardening-2026-09
```
...
1. **`app\google-services.json`** — from the backup if you have it, otherwise download
...
`app\build\outputs\apk\debug\app-debug.apk`. If Gradle fails oddly right after a fresh
...
```
Read docs/HANDOFF.md, then docs/PROJECT-STATUS.md, then continue.
```
